...
class InvoicesService:
    ...
    def get_invoices(self):
        ...
        url = self._user.get_absolute_url('/api/Invoices')
        try:
            response = requests.get(url, headers=headers, timeout=10, verify=False)
            logger.info(f"پاسخ دریافت فاکتورها: کد وضعیت {response.status_code}, محتوا: {response.text}")
            if response.status_code in (200, 201):
                ...
                raise Exception(f"خطا در دریافت فاکتورها: {error_message}")
