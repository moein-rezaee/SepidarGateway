...
class CustomersService:
    ...
    def get_customers(self):
        ...
        url = self._user.get_absolute_url('/api/Customers')
        try:
            response = requests.get(url, headers=headers, timeout=10, verify=False)
            logger.info(f"پاسخ دریافت مشتریان: کد وضعیت {response.status_code}, محتوا: {response.text}")
            if response.status_code in (200, 201):
                ...
                raise Exception(f"خطا در دریافت مشتریان: {error_message}")
